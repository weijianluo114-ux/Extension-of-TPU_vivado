...
MANT_WIDTH+1
    ...
    //对非规格数的判断
    assign is_abnorm_A_stage1 = is_exp_zero_A_stage1 && !(is_frac_zero_A_stage1);
    ...
    always @(posedge clk or negedge rst_n) begin
        if (!rst_n) begin
            ...
            if (is_NaN_stage2) begin
                ...
                //若不是则进行其它操作
                is_nan_stage3 <= 1'b0;  //记得在0的时候要将其清0
            end else begin
    ...
    always @(posedge clk or negedge rst_n) begin
        ...
        else if (sum_stage3[MANT_WIDTH] == 1'b1)
        begin// 最高位溢出（进位产生1）,右移尾数
                    // 溢出成为无穷大
    ...
    always @(posedge clk or negedge rst_n) begin
        if (!rst_n) begin
            ...
            if (is_NaN_stage4) begin
                ...
                C <= {
                    result_sign_stage4, {EXP_WIDTH{1'b1}}, {FRAC_WIDTH{1'b0}}
                };  //对应的无穷大
            end else begin  //若都不是则进行其它操作
