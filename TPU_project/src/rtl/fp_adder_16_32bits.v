...
module fp_adder_16_32bits #(  // 输入+三级流水（对阶 加减 规格化）+输出
  ...
  /******************************* 参数 ***********************************/
  localparam QNAN = {1'b0, {31{1'b1}}};  //作为一个默认的NaN输出
  ...
  reg [FP_WIDTH-1:0] NaN_result_stage2;  //传递有一个数是0时的结果
  ...
  reg [FP_WIDTH-1:0] NaN_result_stage3;  //传递有一个数是0时的结果
  ...
  reg [FP_WIDTH-1:0] NaN_result_stage4;  //传递有一个数是0时的结果
  ...
  always @(posedge clk or negedge rst_n) begin
    if (!rst_n) begin  // 复位
      ...
      if (read_en) begin  // read_en有效，初始化
        ...
        //检测
        // 检测A和B的指数是否全1
        is_exp_one_A_stage1 <= A[FP_WIDTH-2:FRAC_WIDTH] == 8'd1 ? 1'b1 : 1'b0;
        is_exp_one_B_stage1 <= B[FP_WIDTH-2:FRAC_WIDTH] == 8'd1 ? 1'b1 : 1'b0;
        // 检测A和B的指数是全0
        is_exp_zero_A_stage1 <= A[FP_WIDTH-2:FRAC_WIDTH] == 8'd0 ? 1'b1 : 1'b0;
        is_exp_zero_B_stage1 <= B[FP_WIDTH-2:FRAC_WIDTH] == 8'd0 ? 1'b1 : 1'b0;
        //检测A和B尾数是否全0
        is_frac_zero_A_stage1 <= A[FRAC_WIDTH-1:0] == 23'd0 ? 1'b1 : 1'b0;
        is_frac_zero_B_stage1 <= B[FRAC_WIDTH-1:0] == 23'd0 ? 1'b1 : 1'b0;
      end else begin
        en_stage1 <= 1'b0;
      end
    end
  ...
  always @(posedge clk or negedge rst_n) begin
    if (!rst_n) begin  // 复位
      ...
      //首先进行是否为0的判断
      if (is_zero_A_stage1) begin  //为0则传递
      ...
      end else if (is_NaN_stage1) begin  //为NaN则传递
        //若都不是则进行其它操作
      end else begin
        //给出默认值，防止锁存
        zero_result_stage2 <= 32'b0;
  ...
  always @(posedge clk or negedge rst_n) begin
    if (!rst_n) begin
      ...
      //首先进行是否为0的判断
      if (is_zero_stage2) begin  //为0则传递
        //若不是则进行其它操作
      end else if (is_NaN_stage2) begin
        //若不是则进行其它操作
      end else begin
        if (is_inf_A_stage2 || is_inf_B_stage2) begin
          ...
          if (is_inf_A_stage2 && is_inf_B_stage2 && (signA_stage2 != signB_stage2)) begin
            ...
            is_nan_stage3 <= 1'b0;
            // 结果符号为无穷大的符号
            if (is_inf_A_stage2 && !is_inf_B_stage2) result_sign_stage3 <= signA_stage2;
            else if (!is_inf_A_stage2 && is_inf_B_stage2) result_sign_stage3 <= signB_stage2;
  ...
  always @(posedge clk or negedge rst_n) begin
    if (!rst_n) begin  //复位
      ...
      is_NaN_stage4      <= is_NaN_stage3;  //传递
      zero_result_stage4 <= zero_result_stage3;


      //首先进行是否为0的判断
      if (is_zero_stage3) begin  //0则传递
        //若不是则进行其它操作
      end else if (is_NaN_stage3) begin
        //若不是则进行其它操作
      end else begin  //若都不是则进行其它操作
        //无穷大/NaN传递
        if (is_inf_stage3 || is_nan_stage3) begin
          exp_stage4 <= exp_stage3;
          if (is_inf_stage3) norm_mantissa <= 0;  // 无穷大的尾数全为0
          else norm_mantissa <= sum_stage3;
          ...
          if (exp_stage3 > shift_amount) begin
  ...
  always @(posedge clk or negedge rst_n) begin
    if (!rst_n) begin
      ...
      if (is_zero_stage4) begin  //0则传递
        C <= zero_result_stage4;
      end else if (is_NaN_stage4) begin
        C <= QNAN;
      end else begin  //若都不是则进行其它操作
