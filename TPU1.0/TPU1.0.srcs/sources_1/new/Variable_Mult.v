...
module fpu_mult #(
    ...
    always @(posedge clk or negedge rst_n) begin
        if (!rst_n)
            valid_pipeline <= 3'b000; // 复位时清零
        else if (!en)
            valid_pipeline <= 3'b000; // 禁用时清零
        else
            valid_pipeline <= {valid_pipeline[1:0], en}; // 移位传递有效信号
    end
    ...
    always @(posedge clk or negedge rst_n) begin
        if (!rst_n)
            ...
            r_mult_valid <= valid_pipeline[1]; // 第二级流水线有效信号
    end

    // 输出乘法有效信号延迟一个周期，与结果同步
    always @(posedge clk)
        mult_valid <= r_mult_valid;
        ...
        if (one_m_reg == 0 && !zero_times_inf_stage2) begin
            ...
            if (round_cfg) begin
                ...
                if (mul_out_p[MANT_WIDTH] && (mul_out_p[MANT_WIDTH-1] || |mul_out_p[MANT_WIDTH-2:0]))
                    ...
                    two_m_out = mul_out_p[2*(MANT_WIDTH+1)-3:MANT_WIDTH]; // 截断
            end else begin
                // 截断模式：直接取高位
